...
- The accepted source-native Promise Progress product is active in memory and fixtures, but has no workbook bridge. Summary/BS golden `summary-bs-source-native:anf@1.0.0` and Valuation golden `valuation-source-native:anf@1.0.0` are accepted with lossless scratch replay bridges that remain `target_not_wired`, `production_default=false`. The normalized/frozen-shell engine is a validated transition path, not a universal production replacement.
