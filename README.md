...
- The accepted source-native Promise Progress product is active in memory and fixtures, but has no workbook bridge. Summary/BS golden `summary-bs-source-native:anf@1.0.0`, historical Valuation golden `valuation-source-native:anf@1.0.0`, and Capital Allocation / Return golden `capital-allocation-return-source-native:anf@1.0.0` are accepted. The current Capital product workbook identity is `valuation-source-native-workbook:anf@2.0.0`; every replay bridge remains `target_not_wired`, `production_default=false`. The normalized/frozen-shell engine is a validated transition path, not a universal production replacement.
