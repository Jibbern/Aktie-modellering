...
- The accepted source-native Promise Progress product is active in memory and fixtures, but has no workbook bridge. Summary/BS golden `summary-bs-source-native:anf@1.0.0`, historical Valuation golden `valuation-source-native:anf@1.0.0`, Capital Allocation / Return golden `capital-allocation-return-source-native:anf@1.0.0`, and cross-ticker Operating Drivers golden `operating-drivers-source-native:cross-ticker@1.0.0` are accepted. Operating Drivers workbook goldens are registered independently for ANF, PBI, and GPRE; every replay bridge remains `target_not_wired`, `production_default=false`. The normalized/frozen-shell engine is a validated transition path, not a universal production replacement.
