...
- Some compatibility writer paths swallow failures or continue after them, and legacy workbook publication is not universally atomic. Both are deferred production risks; a future source-native workbook bridge must use explicit failures and validation-before-publication rather than copy these behaviors.
