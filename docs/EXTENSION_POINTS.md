...
The source-native Valuation product is golden accepted as `valuation-source-native:anf@1.0.0`. Its formula-aware replay bridge remains `target_not_wired` and is not a production default; route economic/formula-ownership changes through `concept:valuation-economics@1` and binding/materialization/calculation-metadata changes through `concept:valuation-workbook-projection@1`.
...
| Valuation economic, definition or formula-owner change | `concept:valuation-economics@1`; `change:product-field@1`, `change:free-cash-flow-definition@1` or `change:net-debt-definition@1` as applicable | immutable Valuation plan, canonical Investment Case outputs/names, formula ownership, definition/basis and missing-state contract | 930-consumer readback, 20-link/40-name/24-row reconciliation, native formula/value replay and golden fixture | duplicate workbook engine, legacy formula resurrection, writer-only source fallback | product-contract and semantic-golden gates; canonical-owner gate when ownership changes |
| Valuation workbook mutation or calculation metadata | `concept:valuation-workbook-projection@1`; `change:workbook-destination-binding@1` | immutable mutation plan, formula-aware lossless materializer, `valuation-native-safe-calculation-metadata@1`, protected Summary/BS base identity | lossless preservation, deterministic replay, two-run/two-pass native gate, visual receipt | artifact-tool XLSX export, post-open COM repair, production route, protected workbook overwrite | presentation/layout gate; workbook-cutover gate before any production route |
| Capital Allocation / Return economic or period contract | `concept:capital-allocation-economics@1`; `change:product-field@1` or `change:metric-definition@1` | typed activity family, canonical-owner route, semantic period, relevance, derivation, missing state, lineage | buyback/dividend/issuance/no-activity profiles, generic periods, missing-to-zero mutation, ANF golden replay, PBI smoke where relevant | ticker-specific economic branch, workbook row position, unsupported zero, PBI production binding | product-contract and semantic-golden gates; canonical-owner gate when owner routing changes |
...
- The Promise Progress source-native-to-workbook bridge is not implemented. The accepted Summary/BS, Valuation, and Capital Allocation / Return lossless bridges can reproduce their goldens into scratch output, but remain `target_not_wired` and are not production routes.
...
- **Target not wired:** designed or replayable boundary with no active production producer/consumer connection. Promise Progress integration and the accepted-but-nonproduction Summary/BS, Valuation, and Capital Allocation / Return bridges are in this state.
