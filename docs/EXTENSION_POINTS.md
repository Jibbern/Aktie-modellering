...
- The Promise Progress source-native-to-workbook bridge is not implemented. The accepted Summary/BS and Valuation lossless bridges can reproduce their goldens into scratch output, but remain `target_not_wired` and are not production routes.
...
- **Target not wired:** designed or replayable boundary with no active production producer/consumer connection. Promise Progress integration and the accepted-but-nonproduction Summary/BS and Valuation bridges are in this state.
