...
Every canonical FiscalCalendar owns one explicit versioned `calendar_rule_id`; Period retains only `calendar_id` and resolves the rule through that canonical calendar. The closed rule registry supports `rule:core:source-labelled-52-53-week-fiscal@1` with strict equal-duration quarter comparisons and `rule:core:calendar-year-fiscal@1` with exact January-March, April-June, July-September and October-December boundaries. Calendar-year QoQ and YoY may differ naturally in day count only after both periods independently satisfy those exact boundaries, sequencing and ordinal rules. Missing, malformed, unknown or conflicting rule identities fail closed and are never inferred from issuer, ticker, calendar ID, duration, dates or display hints. Construction and semantic validation call the same pure compatibility implementation and validator replay never trusts the serialized comparability result.

The canonical v1 schema requires `calendar_rule_id` for newly generated calendars. No legacy compatibility loader existed when this field was introduced, the sidecar was not yet production authority, and all known canonical producers are migrated together; consequently schema version `1.0.0` remains unchanged rather than creating an unused parallel v2 authority.
