...
| --- | --- | --- | --- | --- | --- | --- | --- |
| summary_company_description_value | SUMMARY | yes | same | same | same | same | - |
| summary_strategic_context_value | SUMMARY | yes | same | same | same | same | - |
| summary_key_advantage_value | SUMMARY | yes | same | same | same | same | - |
...
| summary_wrong_if_values | SUMMARY | yes | same | same | same | same | - |
...
| bs_segment_quarterly_values | BS_Segments | yes | same | similar | similar | same | ANF: source sheet shorter than block range, GPRE: source sheet shorter than block range |
| bs_segment_annual_values | BS_Segments | yes | missing | similar | same | same | ANF: missing block or sheet, ANF: source sheet shorter than block range, PBI: source sheet shorter than block range, +2 more |
| od_watchlist_values | Operating_Drivers | yes | same | similar | similar | same | GPRE has separate sector overlays that are excluded from standard blocks |
...
| ic_key_debate_values | {ticker}_Investment_Case | yes | same | similar | similar | same | - |
...
| ic_lower_brand_health_values | {ticker}_Investment_Case | yes | missing | similar | similar | same | PBI: missing block or sheet, PBI: source sheet shorter than block range, GPRE: missing block or sheet, +1 more |
...
| ic_lower_store_productivity_values | {ticker}_Investment_Case | yes | missing | similar | different | same | PBI: missing block or sheet, PBI: source sheet shorter than block range, GPRE: missing block or sheet, +1 more |
...
| qn_quarter_summary_values | Quarter_Notes_UI | yes | same | same | same | same | - |
| qn_quarter_block_values | Quarter_Notes_UI | yes | same | same | different | same | ANF: source sheet shorter than block range, PBI: source sheet shorter than block range, GPRE: source sheet shorter than block range |
...
| pp_guidance_timeline_values | Promise_Progress_UI | yes | missing | similar | different | same | GPRE: missing block or sheet, GPRE: source sheet shorter than block range |
