...
def parse_debt_summary_from_primary_doc(
    html_bytes: bytes,
    *,
    quarter_end: Optional[dt.date] = None,
) -> Dict[str, Any]:
    """Parse principal, carrying-value, and financing-cost debt disclosures."""
    html = html_bytes.decode("utf-8", errors="ignore")
    tables = read_html_tables_any(html_bytes)
    scale = _detect_scale(html)
    tranches: List[Dict[str, Any]] = []
    carrying_by_kind: Dict[str, Dict[str, float]] = {}
    principal_total: Optional[float] = None
    unamortized_costs: Optional[float] = None
    current_portion: Optional[float] = None
    ...
    def _row_cells(row: pd.Series) -> List[str]:
        return [_clean_cell(value) for value in row.tolist()]

    def _label(cells: List[str]) -> str:
        for cell in cells:
            if re.search(r"[A-Za-z]", cell) and not re.fullmatch(
                r"(?:SOFR|LIBOR)(?:\s+plus\s+\d+\s+bps)?",
                cell,
                re.IGNORECASE,
            ):
                return cell
    ...
    def _amounts(cells: List[str], label: str) -> List[float]:
        out: List[float] = []
        label_consumed = False
        for cell in cells:
            if not cell:
                continue
            if not label_consumed and cell == label:
                label_consumed = True
                continue
            low = cell.lower()
            if re.search(r"\b(?:sofr|libor|interest rate|bps)\b|%", low):
                continue
            if re.fullmatch(r"\d{1,2}/\d{1,2}/20\d{2}", cell):
                continue
            if _is_currency_placeholder_cell(cell):
                continue
            accounting_negative = re.fullmatch(
                r"\(\s*(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*\)",
                cell,
            )
            if accounting_negative:
                value = -float(accounting_negative.group(1).replace(",", ""))
            else:
                value = coerce_number(cell)
            if value is None:
                continue
            if 1900 <= abs(float(value)) <= 2100 and float(value).is_integer():
                continue
    ...
    for table in tables:
        if table is None or table.empty:
            continue
        table_blob = " ".join(
            [*(str(c) for c in table.columns), *_stringify_table_cells(table.values.ravel().tolist())]
        )
        table_low = table_blob.lower()

        is_principal_table = (
            "total principal outstanding" in table_low
            and "unamortized deferred financing costs" in table_low
        )
        is_carrying_table = (
            "carrying value" in table_low
            and "fair value" in table_low
            and ("term loan" in table_low or "senior notes" in table_low)
        )
        if not is_principal_table and not is_carrying_table:
            continue
        if quarter_end is not None and period_match is not True:
            if re.search(
                rf"\b{quarter_end.strftime('%B')}\s+{quarter_end.day},?\s+{quarter_end.year}\b",
                table_blob,
                re.IGNORECASE,
            ):
                period_match = True
        ...
        for _, row in table.fillna("").iterrows():
            ...
            if not label:
                continue
            low = label.lower()
            amounts = _amounts(cells, label)
            if not amounts:
                continue

            if is_principal_table:
                if "total principal outstanding" in low:
                    principal_total = abs(float(amounts[0]))
                    continue
                if "unamortized deferred financing costs" in low:
                    unamortized_costs = abs(float(amounts[0]))
                    continue
                if "current portion of long-term debt" in low or "current portion of long term debt" in low:
                    current_portion = abs(float(amounts[0]))
                    continue
                if "total long-term debt" in low or "total long term debt" in low:
                    long_term_debt = abs(float(amounts[0]))
                    continue

                kind = ""
                if "term" in low and ("facility" in low or "loan" in low):
                    kind = "term_loan"
                elif "senior notes" in low:
                    kind = "senior_notes"
                elif low == "other":
                    kind = "other"
                if not kind:
                    continue
                row_text = " ".join(cells)
                maturity_years = re.findall(r"\b(20\d{2})\b", row_text)
                coupon = re.search(r"\b(\d+(?:\.\d+)?)\s*%", row_text)
                tranches.append(
                    {
                        "name": label,
                        "kind": kind,
                        "principal_amount": abs(float(amounts[0])),
                        "maturity_year": int(maturity_years[-1]) if maturity_years else None,
                        "coupon": float(coupon.group(1)) / 100.0 if coupon else None,
                    }
                )
                continue
            ...
            if "term loan" in low:
                ...
                kind = "senior_notes"
            if kind:
                carrying_by_kind[kind] = {
                    "carrying_value": abs(float(amounts[0])),
                    "fair_value": abs(float(amounts[1])) if len(amounts) > 1 else abs(float(amounts[0])),
                }

    for tranche in tranches:
        carrying = carrying_by_kind.get(str(tranche.get("kind") or ""))
        if carrying:
            tranche.update(carrying)
        else:
            tranche["carrying_value"] = None
            tranche["fair_value"] = None

    carrying_value_total: Optional[float] = None
    if principal_total is not None and unamortized_costs is not None:
        carrying_value_total = float(principal_total) - float(unamortized_costs)
    elif long_term_debt is not None and current_portion is not None:
        carrying_value_total = float(long_term_debt) + float(current_portion)

    if not tranches:
        return {}
    return {
        "quarter_end": quarter_end,
        "period_match": period_match if period_match is not None else (False if quarter_end is not None else None),
        "scale": scale,
        "tranches": tranches,
        "principal_total": principal_total,
        "carrying_value_total": carrying_value_total,
        "unamortized_deferred_financing_costs": unamortized_costs,
        "current_portion": current_portion,
        "long_term_debt_carrying_value": long_term_debt,
    }


def parse_gtx_debt_event_from_text(
    text: str,
    *,
    reported_quarter_end: Optional[dt.date] = None,
) -> Dict[str, Any]:
    """Parse Garrett's May 2026 term-loan repayment/repricing event."""
    source = html_lib.unescape(re.sub(r"<[^>]+>", " ", str(text or "")))
    source = re.sub(r"\s+", " ", source).strip()
    if not source:
        return {}

    date_match = re.search(
        r"\b(May)\s+(\d{1,2}),?\s+(20\d{2})\b",
        source,
        re.IGNORECASE,
    )
    repayment_match = re.search(
        r"\$?\s*(\d+(?:\.\d+)?)\s*(?:M|million)\s+early\s+repayment",
        source,
        re.IGNORECASE,
    )
    principal_match = re.search(
        r"existing\s+\$?\s*(\d+(?:\.\d+)?)\s*(?:M|million)\s+term\s+loan",
        source,
        re.IGNORECASE,
    )
    spread_match = re.search(
        r"SOFR[^A-Za-z0-9]{0,12}\s*plus\s+(\d+)\s+basis\s+points",
        source,
        re.IGNORECASE,
    )
    reduction_match = re.search(r"(\d+)[-\s]+basis\s+point\s+reduction", source, re.IGNORECASE)
    if not repayment_match or not principal_match:
        return {}

    repayment = float(repayment_match.group(1)) * 1_000_000.0
    principal_before = float(principal_match.group(1)) * 1_000_000.0
    event_date = None
    if date_match:
        event_date = dt.date(int(date_match.group(3)), 5, int(date_match.group(2)))
    return {
        "event_date": event_date,
        "reported_quarter_end": reported_quarter_end,
        "reported_history_debt_adjustment": 0.0,
        "pro_forma_debt_adjustment": -repayment,
        "term_loan_principal_before": principal_before,
        "term_loan_principal_after": principal_before - repayment,
        "new_spread_bps": int(spread_match.group(1)) if spread_match else None,
        "spread_reduction_bps": int(reduction_match.group(1)) if reduction_match else None,
        "application": "pro_forma_valuation_only",
    }
...
def parse_debt_tranches_from_primary_doc(
    ...
    *,
    ...
    summary = parse_debt_summary_from_primary_doc(
        html_bytes,
        quarter_end=quarter_end,
    )
    fallback_rows: List[Dict[str, Any]] = []
    for tranche in summary.get("tranches", []) if summary else []:
        principal_amount = tranche.get("principal_amount")
        if principal_amount is None or str(tranche.get("kind") or "") == "other":
            continue
        fallback_rows.append(
            {
                ...
                "carrying_value": tranche.get("carrying_value"),
                "fair_value": tranche.get("fair_value"),
                "maturity_year": tranche.get("maturity_year"),
                "coupon": tranche.get("coupon"),
                "row_text": str(tranche.get("name") or ""),
                "amount_col_idx": None,
                "asof_col_date": quarter_end,
                "asof_select_method": "structured_debt_summary_fallback",
    ...
    if len(fallback_rows) >= 2:
        return (
            fallback_rows,
            4,
            summary.get("carrying_value_total"),
            summary.get("long_term_debt_carrying_value"),
            "Total debt carrying value",
            float(summary.get("scale") or global_scale),
            summary.get("period_match"),
        )
