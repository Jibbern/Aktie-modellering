...
def _anf_visible_quarter_label(qd: Any) -> str:
    fiscal_year = _anf_fiscal_year_from_quarter_end(qd)
    fiscal_quarter = _anf_fiscal_quarter_from_quarter_end(qd)
...
def _apply_source_backed_promise_mapping_overrides(wb: Workbook, ticker: Any = "") -> None:
    ...
    def _promise_display_section_from_horizon(stated_in: Any, horizon: Any, fallback_section: str) -> str:
        stated_txt = str(stated_in or "").strip()
    ...
    def _source_date_ordinal(value: Any) -> int:
        try:
            return pd.Timestamp(str(value or "")).date().toordinal()
        except Exception:
            return 0

    def _append_prior_source_note(values: List[Any], existing: Mapping[str, Any]) -> None:
        if len(values) < 11:
            return
    ...
    if ticker_txt == "PBI":
        ...
        if has_pbi_curated_layout:
            ...
            def _pbi_source_record_values(record: Any) -> Tuple[str, str, str, str, str]:
        ...
        q4_adjusted_eps_note = "Q4 actual shown in Actual (adjusted EPS); official FY adjusted EPS shown in Progress / run-rate. FY can differ from summed rounded quarters."
        q4_share_note = "Q4 actual shown in Actual (diluted shares); Progress shows share-count reduction versus guide and YTD."
        for metric_txt, (prev_txt, new_txt, _actual_txt, _progress_txt, status_txt) in anf_final_q4_rows.items():
...
def _anf_valuation_guidance_rows(guidance_df: pd.DataFrame) -> List[Dict[str, str]]:
    if guidance_df is None or guidance_df.empty:
        return []
    try:
        frame = _anf_visible_guidance_normalized_frame(guidance_df)
    except Exception:
    ...
    for _, rr in frame.iterrows():
        horizon = _anf_clean_visible_ui_text(rr.get("horizon_label") or rr.get("period_label") or "")
        stated = _anf_clean_visible_ui_text(rr.get("stated_in_label") or rr.get("source_quarter_label") or "")
        metric = _metric_display(rr.get(metric_col))
        if not horizon or not metric:
            continue
        if horizon not in {"2026 year", "2026-Q1"}:
            continue
        guidance = _anf_format_guidance_display_value(
            metric,
            rr.get("low"),
            rr.get("high"),
            rr.get("value"),
        ...
        rows.append(
            {
                ...
                "metric": metric,
                "guidance": guidance,
                "source_note": _anf_clean_visible_ui_text(rr.get("line") or rr.get("source_context") or rr.get("doc") or "", max_chars=180),
            }
        )
    order = {
...
def _valuation_side_panel_style_bundle() -> Dict[str, Any]:
    return _style_valuation_side_panel_style_bundle()
...
def _write_anf_valuation_side_panel(ws: Any, *, start_row: int = 7, start_col: int = 15, end_col: int = 29) -> Dict[str, int]:
    ...
    if panel_width < 8:
        c2 = c1 + 7
    row = int(start_row)
    style = _valuation_side_panel_style_bundle()
    ...
    def _row_values(values: Sequence[Any], spans: Sequence[Tuple[int, int]], *, fill: Optional[PatternFill] = None, wrap_cols: Set[int] = frozenset(), source_note: str = "") -> int:
        nonlocal row
...
def _anf_visible_quarter_note_summaries(
    ...
    *,
    ...
    current_quarter_label = _anf_clean_visible_ui_text(quarter_label, max_chars=32)
    latest_quarter_label = _anf_clean_visible_ui_text(latest_label, max_chars=32)
    ...
    def _fit_note(note_in: Any) -> str:
        note = _anf_clean_visible_ui_text(note_in, max_chars=220)
        low_note = note.lower()
        ...
        return _anf_clean_visible_ui_text(note, max_chars=145)

    txt = _anf_clean_visible_ui_text(text_in)
    if not txt:
        return []
    low = txt.lower()
...
def _anf_compact_driver_label(label_in: Any, unit_txt: Any = "") -> str:
    label = _anf_clean_visible_ui_text(label_in)
    low = label.lower()
...
def _anf_compact_driver_group(group_in: Any, label_in: Any = "", driver_key: Any = "") -> str:
    blob = glx_normalize_text(" ".join([str(group_in or ""), str(label_in or ""), str(driver_key or "")])).lower()
...
def _anf_polish_quarter_note_visible_fields(category_in: Any, metric_in: Any, note_in: Any) -> Tuple[str, str]:
    note = _anf_clean_visible_ui_text(note_in, max_chars=250)
    category = _anf_clean_visible_ui_text(category_in) or "Other"
    metric = _anf_clean_visible_ui_text(metric_in, max_chars=64)
    low = note.lower()
    ...
    return category, _anf_clean_visible_ui_text(metric, max_chars=64)
...
def _anf_clean_visible_operating_driver_records(rows_in: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _norm(value: Any) -> str:
        return glx_normalize_text(str(value or "")).lower()
    ...
    def _add_clean_text(key: str, group: str, label: str, commentary: str) -> None:
        text = _anf_clean_visible_ui_text(commentary, max_chars=180)
        if not text:
            return
        cleaned.append(
            {
    ...
    for rec in cleaned:
        label = _anf_compact_driver_label(rec.get("Driver"), rec.get("Unit"))
        group = _anf_compact_driver_group(rec.get("Driver group"), label, rec.get("_driver_key"))
        q_ts = pd.to_datetime(rec.get("Quarter"), errors="coerce")
        q_key = str(pd.Timestamp(q_ts).date()) if pd.notna(q_ts) else ""
        val_key = str(_anf_round_visible_driver_value(rec.get("Value"), rec.get("Unit"), label, rec.get("_driver_key")))
...
def _anf_guidance_visible_period_label(period_label: Any, source_quarter: Any = None) -> str:
    ...
    return _shared_visible_period_text(raw)
...
def _anf_visible_guidance_normalized_frame(guidance_df: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    df["period_label"] = [
        _anf_reclassify_guidance_period_label(label, metric, line)
    ...
    for _, rec in df.iterrows():
        ...
        if not _anf_guidance_metric_unit_is_compatible(metric, unit, line):
        ...
        if pd.notna(source_q):
            source_fy_local = _anf_fiscal_year_from_quarter_end(source_q)
            source_q_local = _anf_fiscal_quarter_from_quarter_end(source_q)
    ...
    if df.empty:
        return df
    df["period_label"] = [
        _anf_guidance_visible_period_label(label, q)
        for label, q in zip(df["period_label"].tolist(), df.get("quarter", pd.Series([None] * len(df))).tolist())
    ]
    df["horizon_label"] = df["period_label"]
    df["horizon_type"] = df["horizon_label"].map(_anf_guidance_horizon_type)
    df["stated_in_label"] = [
        _anf_visible_quarter_label(q) if _anf_visible_quarter_label(q) else _anf_clean_visible_ui_text(q)
    ...
    for line, src, dst in zip(df["line"].tolist(), df["_source_period_label"].tolist(), df["period_label"].tolist()):
        ...
        if src_txt and dst_txt:
            ...
            line_txt = re.sub(re.escape(_anf_clean_visible_ui_text(src_txt)), dst_txt, line_txt, flags=re.I)
        ...
        cleaned_lines.append(_anf_clean_visible_ui_text(line_txt))
...
def _anf_format_guidance_value(metric: str, low: Any = None, high: Any = None, value: Any = None, unit: Any = "", numbers: Any = "") -> str:
    metric_low = str(metric or "").lower()
    unit_txt = str(unit or "").strip().lower()
    nums_txt = glx_normalize_text(str(numbers or ""))
...
def _anf_build_guidance_timeline_rows(guidance_df: Optional[pd.DataFrame] = None, hist_df: Optional[pd.DataFrame] = None) -> List[Dict[str, str]]:
    ...
    for stated, _source_q, horizon, metric, _previous, _new, _change, actual, _status, _note in base_rows:
        if (
            ...
            final_annual_actuals[
                (_promise_metric_definition_key(metric), str(horizon or "").strip().lower())
            ] = str(actual or "").strip()
    ...
    return [
        {key: (_anf_clean_visible_ui_text(value) if isinstance(value, str) else value) for key, value in row.items()}
        for row in rows
    ]
...
def _anf_build_promise_progress_sections(guidance_df: Optional[pd.DataFrame], hist_df: Optional[pd.DataFrame] = None) -> Dict[str, List[Dict[str, str]]]:
    g = _anf_visible_guidance_normalized_frame(guidance_df)
    ...
    def _guidance_from_doc(doc_token: str, metric_name: str, year_label: str = "2025 year") -> str:
        ...
        if sub.empty:
            return ""
        rec = sub.iloc[0]
        return _anf_format_guidance_value(
            metric_name,
            rec.get("low"),
            rec.get("high"),
            rec.get("value"),
...
def _slides_guidance_has_explicit_metric(
    ...
    *,
    ...
    if sg.empty:
        return False
    metric_key = _slides_guidance_metric_key(metric_name)
    if metric_key == "adj ebit":
        aliases = {"adj ebit", "operating margin"}
    else:
        aliases = {metric_key}
    for rec in sg.to_dict("records"):
        hint = _slides_guidance_metric_key(rec.get("metric_hint") or rec.get("metric") or "")
...
def build_writer_context(inputs: WorkbookInputs) -> WriterContext:
    ...
    def _build_facts_long() -> pd.DataFrame:
        ...
        base["period_type"] = base.apply(_period_type, axis=1)
        base["source_class"] = base["source"].apply(_source_class)
        base["method"] = base["source"].apply(_source_method)
        base["qa_severity"] = base["source"].apply(_source_qa)
        base["source_tier"] = base["source"].apply(_source_tier)
        ...
        if needs_review is not None and not needs_review.empty:
            ...
            base["qa_flag"] = base.apply(
                lambda r: sev_map.get((r.get("metric"), pd.to_datetime(r.get("quarter"), errors="coerce")), "pass"),
                axis=1,
            )
        facts = base[[
            "metric",
    ...
    def _build_report(statement: str, scale: float = 1e6) -> pd.DataFrame:
        if hist is None or hist.empty:
            return pd.DataFrame()
        lim = _build_lineitem_map()
        lim = lim[lim["statement"] == statement].sort_values(["sort", "metric"]).copy()
        periods = _build_period_index(12)
        if periods.empty:
            return pd.DataFrame()
        period_cols = [pd.to_datetime(p) for p in periods["period_end"]]
        ...
        if audit is not None and not audit.empty:
            ...
            def _report_source_label_from_audit(row: pd.Series) -> str:
        ...
        if statement == "IS":
            for m in ["revenue", "ebitda", "cfo", "capex"]:
                if m in h.columns:
                    ...
                    ttm_vals[m] = pd.to_numeric(last4[m], errors="coerce").sum()
            ...
            for p in period_cols:
                if metric == "fcf":
                    ...
                    if "cfo" in h.columns and "capex" in h.columns:
                        sub = h[h["quarter"] == p]
                        if not sub.empty:
                            cfo = pd.to_numeric(sub["cfo"], errors="coerce").iloc[0]
                            capex = pd.to_numeric(sub["capex"], errors="coerce").iloc[0]
                    ...
                    if metric in h.columns:
                        sub = h[h["quarter"] == p]
                        if not sub.empty:
                            val = pd.to_numeric(sub[metric], errors="coerce").iloc[0]
                row[p.date()] = (val / scale) if pd.notna(val) else None
            if metric == "fcf":
                has_fcf = any(pd.notna(row.get(p.date())) for p in period_cols)
                if has_fcf:
                    row["Source"] = "Derived"
                    row["QA"] = "PASS"
                    row["Tag"] = ""
        ...
        if statement == "IS":
            ...
            # QoQ/YoY rows
            def _growth(series: pd.Series, periods_back: int) -> Dict[str, float]:
                out: Dict[str, float] = {}
    ...
    def _write_report_sheet(name: str, df: pd.DataFrame, scale_label: str) -> None:
        ...
        for _, row in df.iterrows():
            ws.append([None if pd.isna(row.get(c)) else _safe_cell(row.get(c)) for c in df.columns])
        # style
        header_row = 3
        ws.freeze_panes = "A4"
