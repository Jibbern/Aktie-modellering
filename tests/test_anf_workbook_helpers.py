...
from pbi_xbrl.excel_writer_context import (
    ...
    _final_repair_promise_progress_ui,
...
def test_anf_promise_progress_sections_are_clean_and_open_for_2026() -> None:
    ...
    open_metrics = {r["Metric"] for r in open_rows}
    assert "Margin bridge" not in open_metrics
    assert {
        "2026 tariff headwind",
        "Q1 tariff headwind",
        "Q1 freight tailwind",
        "Q1 ERP disruption",
        "Q1 marketing headwind",
...
def test_shared_promise_progress_postprocess_dedupes_all_timeline_metrics() -> None:
    ...
    headers = PROMISE_TIMELINE_TEST_HEADERS
    ...
    rows = [
        [ws.cell(rr, cc).value for cc in range(1, 12)]
        for rr in range(1, ws.max_row + 1)
    ]
...
def test_shared_promise_progress_postprocess_removes_semantic_carry_forward_rows() -> None:
    ...
    assert capex_rows[0][9] == "2025-12-31"
    ...
    assert cost_rows[0][9] == "2025-03-31"
...
def test_shared_promise_progress_postprocess_preserves_true_initial_rows() -> None:
    ...
    out = [ws.cell(5, cc).value for cc in range(1, 12)]
    assert out[0] == "Cost savings target"
    assert out[1] in {"", None}
    assert out[2] == "$75m-$85m"
    assert out[3] == "Initial"
    assert out[4] == ""
    assert out[5] in {"", "not yet measurable", "Q2: not yet measurable"}
    assert "180m-$200m" not in str(out[10])
    assert "run-rate" not in str(out[10]).lower()
...
def test_shared_promise_progress_postprocess_evaluates_completed_horizons_with_actuals() -> None:
    ...
    out = [[ws.cell(rr, cc).value for cc in range(1, 12)] for rr in range(5, 8)]
    by_metric = {str(row[0] or ""): row for row in out}
    assert by_metric["FCF target"][6] == "Beat"
    assert by_metric["Capex"][6] == "Hit"
    assert by_metric["Adjusted EPS"][6] == "Missed"
...
def test_shared_promise_progress_postprocess_does_not_reclassify_expected_milestones() -> None:
    ...
    assert ws.cell(5, 7).value == "On track"
...
def test_shared_promise_progress_rewrite_puts_values_before_metadata_and_newest_first() -> None:
    ...
    rows = [
        ...
        ["Q4 margin actual", "12.5%", "12.5%", "Completed", "2025-Q4 margin actual result.", "2025-Q4", "2025-Q4", "2025-Q4", "2025-12-31", ""],
        ["FCF guidance", "$340m-$370m", "not yet measurable", "Open", "2026 year FCF guidance $340m-$370m.", "2025-Q4", "2025-Q4", "2026-Q1", "2026-03-31", ""],
    ...
    timeline_metric_rows = [
        ...
        and re.fullmatch(r"\d{4}-\d{2}-\d{2}", str(row[9] or ""))
    ]
    assert timeline_metric_rows[0][8] == "2026-Q1"
    assert not any(row[0] == "Revenue guidance" and row[8] == "2025-Q4" and row[7] == "2026 year" for row in timeline_metric_rows)
    assert timeline_metric_rows[-1][8] == "2025-Q3"
    ...
    assert ws.column_dimensions["F"].width >= 28
    assert ws.column_dimensions["K"].width >= 40


def test_gpre_45z_guidance_without_explicit_2026_does_not_leak_into_future_horizon() -> None:
    wb = Workbook()
    ws = wb.active
    ws.title = "Promise_Progress_UI"
    rows = [
        ...
        [
            ...
            "$55.2m Q1 realized",
            "Completed",
            "45Z EBITDA guidance discussed without a future-year horizon in this 2025-Q2 block.",
            "2025-Q2",
            "2025-Q2",
            "2025-Q2",
            "2025-06-30",
    ...
    values = [[ws.cell(rr, cc).value for cc in range(1, 12)] for rr in range(1, ws.max_row + 1)]
    timeline_row = next(row for row in values if row[0] == "45Z EBITDA guidance" and row[9] == "2025-06-30")
    assert timeline_row[7] == "2025-Q2"
    assert timeline_row[4] in {"", None, "not yet measurable"}
    assert "$55.2m" in str(timeline_row[5] or "")
    assert timeline_row[6] in {"Open", "On track"}
    assert not any(row[0] == "2026 open guidance" for row in values)
...
def test_shared_promise_progress_rewrite_drops_blank_rows_without_creating_q4_actual_rows() -> None:
    ...
    ws.title = "Promise_Progress_UI"
    history = wb.create_sheet("History_Q")
    history.append(["quarter", "revenue", "cfo", "capex", "fiscal_year", "fiscal_quarter", "fiscal_label"])
    history.append([dt.datetime(2025, 3, 31), 480_000_000, 20_000_000, 5_000_000, 2025, 1, "2025-Q1"])
    history.append([dt.datetime(2025, 6, 30), 470_000_000, 120_000_000, 10_000_000, 2025, 2, "2025-Q2"])
    history.append([dt.datetime(2025, 9, 30), 460_000_000, 95_000_000, 15_000_000, 2025, 3, "2025-Q3"])
    history.append([dt.datetime(2025, 12, 31), 482_000_000, 135_000_000, 23_000_000, 2025, 4, "2025-Q4"])
    rows = [
        ["Promise Progress"],
        ["Generated at 2026-05-15 00:00:00 UTC | Quarter blocks"],
        ["Promise progress (As of 2025-12-31)"],
        ...
        ["Promise progress (As of 2025-09-30)"],
        ["Metric", "Target", "Latest", "Result", "Rationale", "Stated", "Last Seen", "Carried To", "Evaluated Through", "Evidence"],
        ["FCF target", "$330m-$370m", "$317m", "On track", "2025 year FCF target tracking.", "2025-Q3", "2025-Q3", "2025-Q3", "2025-12-31", ""],
    ]
    for rr, row in enumerate(rows, start=1):
        for cc, value in enumerate(row, start=1):
            ws.cell(rr, cc, value)
...
def test_shared_promise_postprocess_replaces_latest_actuals_without_adding_q4_rows() -> None:
    ...
    history = wb.create_sheet("History_Q")
    history.append(["quarter", "revenue", "fiscal_year", "fiscal_quarter", "fiscal_label"])
    history.append([dt.datetime(2025, 3, 31), 493_420_000, 2025, 1, "2025-Q1"])
    history.append([dt.datetime(2025, 6, 30), 461_909_000, 2025, 2, "2025-Q2"])
    history.append([dt.datetime(2025, 9, 30), 459_675_000, 2025, 3, "2025-Q3"])
    history.append([dt.datetime(2025, 12, 31), 477_625_000, 2025, 4, "2025-Q4"])
    adjusted = wb.create_sheet("Adjusted_Metrics")
    adjusted.append(["quarter", "adj_ebit", "adj_ebitda", "adj_eps", "adj_fcf", "period_type"])
    adjusted.append([dt.datetime(2025, 3, 31), 119_689_000, 148_013_000, 0.33, -16_679_000, "quarter"])
    adjusted.append([dt.datetime(2025, 6, 30), 102_293_000, 131_055_000, 0.27, 111_388_000, "quarter"])
    ...
    rows = [
        ...
        ["2025-Q4 revisions"],
        headers,
        ["Revenue guidance", "", "$1.76bn-$1.86bn", "Initial", "", "", "Open", "2026 year", "2025-Q4", "2025-12-31", "Initial 2026 guide."],
        ["2025-Q3 revisions"],
        headers,
        ["Revenue guidance", "$1.9bn-$1.95bn", "$1.9bn-$1.95bn", "Maintained", "$1.89bn", "", "Missed", "2025 year", "2025-Q3", "2025-09-30", "Later final actual was incorrectly carried into Q3."],
        ["Adjusted EBIT guidance", "$450m-$465m", "$450m-$465m", "Maintained", "$461.3m", "", "Hit", "2025 year", "2025-Q3", "2025-09-30", "Later final actual was incorrectly carried into Q3."],
        ["2024-Q4 revisions"],
        headers,
        ["Revenue guidance", "", "", "", "", "", "", "", "", "", ""],
    ]
    for rr, row in enumerate(rows, start=1):
        if len(row) == 1 and str(row[0]).endswith("revisions"):
...
def test_pbi_source_backed_promise_overrides_use_company_defined_fcf_and_cost_history() -> None:
    ...
    timeline_headers = PROMISE_TIMELINE_TEST_HEADERS
    rows = [
        ["Promise Progress"],
...
def test_promise_cleanup_removes_actual_only_rows_and_styles_a_to_j() -> None:
    ...
    assert all(str(ws.cell(header_row, cc).fill.fgColor.rgb or "") not in {"00000000", "00FFFFFF"} for cc in range(1, 12))
    assert any(rng.min_row == header_row and rng.max_row == header_row and rng.min_col == 11 and rng.max_col == 12 for rng in ws.merged_cells.ranges)
    assert all(ws.cell(body_row, cc).border.bottom.style == "thin" for cc in range(1, 13))
    assert all(str(ws.cell(body_row, cc).fill.fgColor.rgb or "") not in {"00000000"} for cc in range(1, 13))
...
def test_final_promise_cleanup_removes_blank_v2_rows_and_empty_revision_blocks(tmp_path) -> None:
    ...
    rows = [
        ...
        ["2025-Q3 revisions"],
        headers,
        ["FCF target", "$330m-$370m", "$330m-$370m", "Maintained", "", "$317m", "On track", "2025 year", "2025-Q3", "2025-09-30", "2025 year FCF target tracking."],
    ]
    for rr, row in enumerate(rows, start=1):
        if len(row) == 1 and str(row[0]).endswith("revisions"):
            ws.cell(rr, 1, row[0]).fill = section_fill
    ...
    values = [[ws.cell(rr, cc).value for cc in range(1, 12)] for rr in range(1, ws.max_row + 1)]
    assert not any(row[0] == "FCF target" and all(value in {None, ""} for value in row[1:]) for row in values)
    assert not any(row[0] == "FCF target" and row[8] == "2025-Q4" for row in values)
    assert not any(row[0] == "2025-Q4 revisions" for row in values)

    out_path = tmp_path / "promise_cleanup.xlsx"
    wb.save(out_path)
    reloaded = load_workbook(out_path, data_only=False)
    ws_reloaded = reloaded["Promise_Progress_UI"]
    values_reloaded = [[ws_reloaded.cell(rr, cc).value for cc in range(1, 12)] for rr in range(1, ws_reloaded.max_row + 1)]
    assert not any(row[0] == "FCF target" and all(value in {None, ""} for value in row[1:]) for row in values_reloaded)
    assert not any(row[0] == "FCF target" and row[8] == "2025-Q4" for row in values_reloaded)
    assert not any(row[0] == "2025-Q4 revisions" for row in values_reloaded)


def test_shared_promise_ui_polish_keeps_headers_wide_and_scorecard_compact() -> None:
    wb = Workbook()
    ws = wb.active
    ws.title = "Promise_Progress_UI"
    section_fill = PatternFill("solid", fgColor="5B9BD5")
    rows = [
        ...
        ["Category", "Score", "Evidence", "", "", "", "Read", "", "", ""],
        ["FCF delivery", "Mixed", "FCF is central to the turnaround and debt case.", "", "", "", "Cash conversion must remain credible.", "", "", ""],
    ...
    for row_idx in section_rows:
        assert any(rng.min_row == row_idx and rng.max_row == row_idx and rng.min_col == 1 and rng.max_col == 12 for rng in merged_ranges)
    timeline_header_row = next(
        rr
        for rr in range(1, ws.max_row + 1)
        if ws.cell(rr, 1).value == "Metric" and ws.cell(rr, 2).value == "Previous guide"
    )
    assert all(str(ws.cell(timeline_header_row, col_idx).fill.fgColor.rgb or "").upper() not in {"00000000", "00FFFFFF"} for col_idx in range(1, 12))
    assert any(rng.min_row == timeline_header_row and rng.max_row == timeline_header_row and rng.min_col == 11 and rng.max_col == 12 for rng in ws.merged_cells.ranges)
    ...
    assert float(ws.column_dimensions["C"].width or 0) >= 32.0
    assert float(ws.column_dimensions["F"].width or 0) >= 28.0
    assert float(ws.column_dimensions["K"].width or 0) >= 40.0
...
def test_final_promise_cleanup_adds_latest_pbi_cost_savings_run_rate_row() -> None:
    ...
    rows = [
        ...
        ["2026-Q1 revisions"],
        PROMISE_TIMELINE_TEST_HEADERS,
        ["Revenue guidance", "$1.76bn-$1.86bn", "$1.8bn-$1.86bn", "Updated", "$477.4m", "", "On track", "2026 year", "2026-Q1", "2026-03-31", "2026 revenue guide update."],
    ...
    for rr, row in enumerate(rows, start=1):
        if len(row) == 1 and str(row[0]) in {"2026 open guidance", "Quarterly guidance timeline / revision log", "2026-Q1 revisions"}:
            ws.cell(rr, 1, row[0]).fill = section_fill
            continue
        for cc, value in enumerate(row, start=1):
            ws.cell(rr, cc, value)
    ...
    q1_cost_idx = next(idx for idx, row in enumerate(values, start=1) if row[0] == "Cost savings target" and row[8] == "2025-Q1")
    assert q4_header_idx < q1_header_idx < q1_cost_idx
...
def test_promise_actual_lookup_prefers_fiscal_label_over_calendar_quarter() -> None:
    ...
    ws.cell(2, 1, "2026-Q1 revisions").fill = section_fill
    ws.append(PROMISE_TIMELINE_TEST_HEADERS)
    ...
    assert ws.cell(4, 7).value == "Open"
    assert ws.cell(5, 5).value in {None, ""}
    assert ws.cell(5, 7).value == "Open"
...
def test_shared_promise_postprocess_keeps_cost_savings_run_rate_and_debt_actuals() -> None:
    ...
    rows = [
        ...
        ["2025-Q1 revisions"],
        headers,
        ["Cost savings target", "", "$180m-$200m", "Initial", "", "", "On track", "2025-Q1", "2025-Q1", "2025-03-31", "Annualized savings target; latest disclosed $157m run-rate."],
        ["2025-Q3 revisions"],
        headers,
        ["Debt reduction", "", "$130.7m", "Initial", "", "", "Completed", "2025-Q3", "2025-Q3", "2025-09-30", "Sale proceeds used to repay junior mezzanine debt."],
    ]
    for rr, row in enumerate(rows, start=1):
        if len(row) == 1 and str(row[0]).endswith("revisions"):
            ws.cell(rr, 1, row[0]).fill = section_fill
    ...
    values = [[ws.cell(rr, cc).value for cc in range(1, 12)] for rr in range(1, ws.max_row + 1)]
    cost = next(row for row in values if row[0] == "Cost savings target")
    assert cost[4] in {"", None}
    assert "Run-rate: $157m" in cost[5]
    assert cost[6] == "On track"
    debt = next(row for row in values if row[0] == "Debt reduction")
    assert debt[4] == "Debt repaid"
    assert debt[6] == "Completed"


def test_anf_guidance_timeline_keeps_q4_actuals_out_of_special_annual_rows() -> None:
    hist = pd.DataFrame(
        [
            {
                "quarter": dt.date(2025, 2, 1),
                "revenue": 1_584_917_000.0,
                "op_income": 256_064_000.0,
                "capex": 50_863_000.0,
            },
            {
                ...
                "capex": 76_028_000.0,
                "shares_diluted": 48_500_000.0,
                "buybacks_cash": 450_000_000.0,
            },
        ]
    )
...
def test_shared_promise_progress_rewrite_keeps_future_annual_guidance_open_or_on_track() -> None:
    ...
    assert timeline_header[5] == "Progress / run-rate"
    assert "Actual / latest actual" not in timeline_header
    annual_row = next(row for row in values if row[0] == "2026 year 45Z EBITDA guidance" and row[7] == "2026 year")
    assert annual_row[6] in {"On track", "Open"}
    assert annual_row[6] != "Completed"
...
def test_shared_ui_polish_cleans_guidance_normalized_and_downgrades_pbi_qsum() -> None:
    ...
    wb_anf = Workbook()
    wb_anf.active.title = "Valuation"
    anf_needs = wb_anf.create_sheet("Needs_Review")
    anf_needs.append(["priority", "severity", "check", "latest_message", "first_seen", "last_seen", "recommended_action"])
    anf_needs.append(["P1", "fail", "qsum_vs_fy", "Sum of 4 quarters vs FY fact.", "2019-02-02", "2019-02-02", ""])

    _apply_shared_ui_conventions_to_workbook(wb_anf, ticker="ANF")

    assert anf_needs["A2"].value == "P2"
    assert anf_needs["B2"].value == "warn"
    assert "older fiscal/YTD" in str(anf_needs["G2"].value)
...
def test_shared_ui_polish_standardizes_promise_statuses_and_valuation_row_heights() -> None:
    ...
    promise.append([])
    promise.append(PROMISE_TIMELINE_TEST_HEADERS)
    promise.append(["Revenue", "+3-5%", "+4-6%", "on track", "$1.2bn", "", "open", "2025 year", "2025-Q1", "2025-03-31", "Fixture row."])
    ...
    assert promise.cell(timeline_row, 7).value == "Open"
    assert promise.cell(timeline_row, 4).fill.fgColor.rgb != promise.cell(timeline_row, 7).fill.fgColor.rgb
    assert promise.cell(timeline_row, 5).fill.fgColor.rgb != promise.cell(timeline_row, 7).fill.fgColor.rgb
    assert promise.cell(timeline_row, 6).fill.fgColor.rgb != promise.cell(timeline_row, 7).fill.fgColor.rgb
...
def test_anf_investment_case_writer_applies_readable_style_and_formula_grid() -> None:
    ...
    header_map = {str(header): idx for idx, header in enumerate(headers, start=1)}
    audit_rows = {
        str(data_ws.cell(r, header_map["metric"]).value): r
        for r in range(2, data_ws.max_row + 1)
        if data_ws.cell(r, header_map["section"]).value == "Scenario Driver Bridge Audit"
    }
    expected_audit_metrics = {
        ...
        "Total bps",
        "Calculated $m impact",
        "Bridge active / guide check",
    }
    assert expected_audit_metrics.issubset(audit_rows)

    case_rows = {
        label: next(r for r in range(1, ws.max_row + 1) if ws.cell(r, 1).value == label)
        for label in [
            "Forward revenue",
            "Tariff impact (bps)",
            "Freight tailwind (bps)",
            "ERP disruption (bps)",
            "Marketing headwind (bps)",
            "Margin bridge vs baseline",
        ]
    }
    value_col = header_map["value"]
    assert data_ws.cell(audit_rows["Active revenue used"], value_col).value == f"='ANF_Investment_Case'!$G${case_rows['Forward revenue']}"
    assert data_ws.cell(audit_rows["Tariff bps"], value_col).value == f"='ANF_Investment_Case'!$G${case_rows['Tariff impact (bps)']}"
    assert data_ws.cell(audit_rows["Freight bps"], value_col).value == f"='ANF_Investment_Case'!$G${case_rows['Freight tailwind (bps)']}"
    assert data_ws.cell(audit_rows["ERP bps"], value_col).value == f"='ANF_Investment_Case'!$G${case_rows['ERP disruption (bps)']}"
    assert data_ws.cell(audit_rows["Marketing bps"], value_col).value == f"='ANF_Investment_Case'!$G${case_rows['Marketing headwind (bps)']}"
    assert data_ws.cell(audit_rows["Total bps"], value_col).value == (
        f"=SUM(C{audit_rows['Tariff bps']}:C{audit_rows['Marketing bps']})"
    )
    assert data_ws.cell(audit_rows["Calculated $m impact"], value_col).value == (
        f"=C{audit_rows['Active revenue used']}*C{audit_rows['Total bps']}/10000"
    )
    assert data_ws.cell(audit_rows["Bridge active / guide check"], value_col).value == (
        f"='ANF_Investment_Case'!$C${case_rows['Margin bridge vs baseline']}"
    )
    assert data_ws.cell(audit_rows["Tariff bps"], header_map["source_note"]).value == "Headwind is negative."
    assert data_ws.cell(audit_rows["Freight bps"], header_map["source_note"]).value == "Tailwind is positive."
    assert data_ws.cell(audit_rows["Total bps"], header_map["source_note"]).value == "Tailwinds positive; headwinds negative."
...
def test_investment_case_manual_inputs_drive_market_and_scenario_sections() -> None:
    sector_df = pd.DataFrame(
        [
            ...
            {"section": "FCF Yield Implied Equity Value", "metric": "5.0% FCF yield", "display": "$100m equity / share $10"},
            {
                ...
                "margin_conversion": 0.12,
                "margin_basis": "Company operating margin proxy",
                "feeds_bridge": "Yes",
                "source_note": "Company operating margin proxy",
            },
            {
                "section": "Segment Scenario Inputs",
                "metric": "SendTech",
    ...
    anf_df = pd.DataFrame(
        [
            ...
            {"section": "FCF Yield Implied Equity Value", "metric": "5.0% FCF yield", "display": "$7,567m equity value", "share_price": 168.2},
            {
                ...
                "margin_basis": "",
                "feeds_bridge": "No",
                "source_note": "Company operating margin proxy; separate revenue cut",
            },
            {
                "section": "Segment Scenario Inputs",
    ...
    for ticker, wb, ws in cases:
        ...
        assert [driver_audit.cell(1, c).value for c in range(1, 14)] == [
            "Ticker",
            "Section",
            "Segment / category",
            "Type",
        ...
        driver_audit_rows = [
            [driver_audit.cell(r, c).value for c in range(1, 14)]
        ...
        next_section_row = next(
            r
            for r in range(manual_row + 2, ws.max_row + 1)
            if str(ws.cell(r, 1).value or "") in {"Segment Scenario Inputs", "Scenario Driver Bridge", "What Market Is Pricing"}
        )
        tax_rate_row = next(r for r in range(manual_row + 2, next_section_row) if ws.cell(r, 1).value == "Scenario tax rate")
        assert tax_rate_row == next_section_row - 2
        ...
        operating_margin_row = next(r for r in range(manual_row + 2, ws.max_row + 1) if ws.cell(r, 1).value == "Operating margin")
        ...
        assert ws.cell(operating_margin_row, 6).fill.fgColor.rgb == "00FFF2CC"
        assert ws.cell(operating_margin_row, 7).number_format == "0.0%"
        assert str(ws.cell(operating_margin_row, 3).value or "") == '=IFERROR(CompanyOperatingMargin_TTM,"")'
        assert str(ws.cell(operating_margin_row, 7).value or "").startswith(
            f'=IF(F{operating_margin_row}<>"",F{operating_margin_row},IF(C{operating_margin_row}<>"",C{operating_margin_row},'
        )
        if ticker != "GPRE":
            ...
            assert ws.cell(operating_margin_row, 2).value == pytest.approx(332.0 / 4600.0)
        ...
        assert f"C{fcf_yield_row}>1" in fcf_yield_formula and f"C{fcf_yield_row}/100" in fcf_yield_formula
        assert ws.cell(fcf_yield_row, 7).number_format == "0.0%"
        ...
        if ticker in {"PBI", "ANF"}:
            ...
            assert ws.cell(segment_row + 1, 9).value == "Notes"
            assert any(rng.min_row == segment_row + 1 and rng.min_col == 9 and rng.max_col == 10 for rng in ws.merged_cells.ranges)
            assert "Revenue % override" not in [ws.cell(segment_row + 1, c).value for c in range(1, 11)]
            assert "EBITDA/EBIT impact" not in [ws.cell(segment_row + 1, c).value for c in range(1, 11)]
            labels = [str(ws.cell(r, 1).value or "") for r in range(segment_row + 2, segment_row + 14)]
            if ticker == "PBI":
                ...
                assert ws.cell(basis_row, 1).value == "Active basis"
                assert ws.cell(basis_row, 2).value == "None"
                assert ws.cell(basis_row, 2).fill.fgColor.rgb == "00FFF2CC"
                assert "None,Brand,Geography" in ws.data_validations.dataValidation[0].formula1
                assert ws.cell(basis_row, 9).value == "Select category"
                brand_header = next(r for r in range(segment_row + 2, ws.max_row + 1) if ws.cell(r, 1).value == "Brand view — summed only when Brand selected")
                geo_header = next(r for r in range(segment_row + 2, ws.max_row + 1) if ws.cell(r, 1).value == "Geography view — summed only when Geography selected")
                assert ws.cell(brand_header, 1).fill.fgColor.rgb == "00DDEBF7"
                assert ws.cell(geo_header, 1).fill.fgColor.rgb == "00DDEBF7"
                ...
                assert str(ws.cell(anf_brand_row, 6).value).startswith("=IF($G$")
            ...
            assert any(row[0] == "GPRE" and row[2] == "Not enabled" and row[11] == "No" for row in driver_audit_rows)
        if ticker == "GPRE":
        ...
        assert ws.cell(bridge_row + 1, 1).value == "Bridge-adjusted values start from active inputs and add incremental effects below | Taxable EPS impacts use active scenario tax rate."
        ...
        assert "Base active values" not in incremental_labels
        assert not any("EBIT_TTM" in str(ws.cell(r, c).value or "") for r in range(1, ws.max_row + 1) for c in range(1, 11))
        bridge_by_label = {
            str(ws.cell(r, 1).value or ""): [str(ws.cell(r, c).value or "") for c in range(1, 9)]
            for r in range(bridge_row + 3, summary_row - 1)
        }
        bridge_row_by_label = {
            str(ws.cell(r, 1).value or ""): r
            for r in range(bridge_row + 3, summary_row - 1)
        }
        audit_by_label = {
        ...
        if ticker == "GPRE":
            ...
            assert f"$G${tax_rate_row}" in segment_bridge_row[4]
            assert f"/$G${shares_row}" in segment_bridge_row[4]
            assert "D" in segment_bridge_row[5]
            segment_bridge_audit = audit_by_label["Selected segment revenue/margin impact"]
            assert segment_bridge_audit[3] == "taxable"
            assert "Segment Scenario Inputs" in str(segment_bridge_audit[5])
            capex_row = bridge_by_label["Capex change vs baseline"]
            ...
            tariff_bps_row = next(r for r in range(manual_row + 2, next_section_row) if ws.cell(r, 1).value == "Tariff impact (bps)")
            freight_bps_row = next(r for r in range(manual_row + 2, next_section_row) if ws.cell(r, 1).value == "Freight tailwind (bps)")
            erp_bps_row = next(r for r in range(manual_row + 2, next_section_row) if ws.cell(r, 1).value == "ERP disruption (bps)")
            marketing_bps_row = next(r for r in range(manual_row + 2, next_section_row) if ws.cell(r, 1).value == "Marketing headwind (bps)")
            ...
            margin_bridge_row_number = bridge_row_by_label["Margin bridge vs baseline"]
            assert "$G$" in str(margin_bridge_row[2])
            assert f"$G${revenue_row}" in str(margin_bridge_row[2])
            for bps_row in (tariff_bps_row, freight_bps_row, erp_bps_row, marketing_bps_row):
                assert f"$G${bps_row}" in str(margin_bridge_row[2])
            ...
            assert f"D{margin_bridge_row_number}" in str(margin_bridge_row[4])
            assert f"D{margin_bridge_row_number}" in str(margin_bridge_row[5])
            assert str(margin_bridge_row[6]) in {"0", "0.0", ""}
            assert "Converts bps margin bridge to $m using active revenue." == margin_bridge_row[7]
            assert ws.cell(tariff_bps_row, 4).value < 0
            assert ws.cell(freight_bps_row, 5).value > 0
            assert ws.cell(erp_bps_row, 5).value < 0
            assert ws.cell(marketing_bps_row, 5).value < 0
            segment_bridge_row = bridge_by_label["Selected segment revenue/margin impact"]
...
def test_anf_valuation_side_panel_scrubber_replaces_strays_with_intentional_blocks() -> None:
    ...
    assert guidance_rows[1] - guidance_rows[0] <= 22
