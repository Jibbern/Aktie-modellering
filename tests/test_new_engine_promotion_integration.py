...
def _integration_context(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    ...
    assert binding_counts["planned_write_count"] == 23_613
    assert binding_counts["structured_skip_count"] == 2_006
    assert binding_counts["issue_count"] == 755
    assert binding_counts["occurrence_count"] == 2_311
    ...
    assert formula_counts["cell_formula_count"] == 2_609
